...
#define N 31624 // This is the maximum N which crosses n*(n-1) value > INF
const Long INF = 1e9 ;
...
void generateNcR(){
	...
	rep(i, N){
		...
		rep(j, size){
		    Long val = P[i][j];
			if( val <= INF && hash.find(val) == hash.end() )
				hash[val] = mp( i, i-j-1 );
		}
	}
}
...
int main(){
    ...
    while( scanf("%lld",&K) != EOF ){
        ...
        else if( hash.find(K) != hash.end() ) printf("Case %d: %d %d\n",cs++, hash[K].ff, hash[K].ss);
