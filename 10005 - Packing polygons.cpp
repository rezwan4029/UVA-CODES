...
     3 parallel
    */
    ...
    {
        return EQ(p.sub(a).det(b.sub(a)))==0&&EQ(p.sub(a).dot(p.sub(b)))<=0;
    }
    bool parallel(line v)
    {
        return EQ(b.sub(a).det(v.b.sub(v.a)))==0;
    }
    /*
     2 norms intersect
...
     0 disjoint / not intersect
    */
    ...
    {
        int d1=EQ(b.sub(a).det(v.a.sub(a)));
        int d2=EQ(b.sub(a).det(v.b.sub(a)));
        int d3=EQ(v.b.sub(v.a).det(a.sub(v.a)));
        int d4=EQ(v.b.sub(v.a).det(b.sub(v.a)));
...
struct circle
{
    ...
    // Do not intersect with both round cx, cy tangent circle radius r1,
    ...
    int pointcrossline(line v,point &p1,point &p2) // (Find the first line post judgment relationseg)
    {
    ...
    // Too little to do tangent to the circle (point and circle first determine the relationship)
    ...
    {
        if (EQ(p.sub(a).det(p.sub(b))==0))return 0.0;
        point q[5];
        int len=0;
        q[len++]=a;
        line l(a,b);
...
struct polygon
{
    ...
    bool isconvex()
    {
        ...
        for (i=0;i<n;i++)
        {
            j=(i+1)%n;
            k=(j+1)%n;
            s[EQ(p[j].sub(p[i]).det(p[k].sub(p[i])))+1]=1;
            if (s[0]&&s[2])return 0;
        }
        return 1;
    }
...
       0 External
    */
    int relationpoint(point q)
    {
        ...
        {
            j=(i+1)%n;
            int k=EQ(q.sub(p[j]).det(p[i].sub(p[j])));
            int u=EQ(p[i].y-q.y);
            int v=EQ(p[j].y-q.y);
            if (k>0&&u<0&&v>=0)cnt++;
            if (k<0&&v<0&&u>=0)cnt--;
       ...
       /* A is positive in the polygon length
         2 or parallel to the side intersection
         0 is no intersection */
    ...
    /* U cut straight left convex polygons
       Note linear direction */
    void convexcut(line u,polygon &po)
    {
        ...
        for (i=0;i<n;i++)
        {
            int d1=EQ(p[i].sub(u.a).det(u.b.sub(u.a)));
            int d2=EQ(p[(i+1)%n].sub(u.a).det(u.b.sub(u.a)));
    ...
    bool getdir() // 1 represents 0 for clockwise counterclockwise
    {
        double sum=0;
        int i;
        for (i=0;i<n;i++)
    ...
    double areacircle(circle c) // area of intersection of polygon and circle
    {
        ...
        {
            int j=(i+1)%n;
            if (EQ(p[j].sub(c.p).det(p[i].sub(c.p)))>=0)
            {
...
      2 is completely inside the circle */
    ...
    int circlecover(double r)//Unit circle cover
    {
        int ans=0,i,j;
    ...
    int pointinpolygon(point q) // (Point inside the convex polygon determination)
    {
        if (getdir())reverse(p,p+n);
        if (EQ(q.sub(p[0]).det(p[n-1].sub(p[0])))==0)
        {
        ...
        {
            mid=(low+high)>>1;
            if (EQ(q.sub(p[0]).det(p[mid].sub(p[0])))>=0&&EQ(q.sub(p[0]).det(p[mid+1].sub(p[0])))<0)
            {
                polygon c;
                c.p[0]=p[mid];
                c.p[1]=p[mid+1];
            ...
            if (EQ(q.sub(p[0]).det(p[mid].sub(p[0])))>0)
            {
                low=mid+1;
            }
            else
...
int main(){
    ...
    while( scanf("%d",&n) && n )
    {
        p.n = n ;
        p.input();
        circle c = p.mincircle();
        double rad = c.r ;
        double r ;
        cin >> r ;
        if( r >= rad ) puts("The polygon can be packed in the circle.");
