...
DB calc_areaofConvex(vector<Point>P)
{
    ...
    DB a = 0.0 ;
    int i,sz  = P.size();
   // REP(i,sz)  a += P[ (i+1)%sz ].y * P[i].x - P[ (i+1)%sz ].x * P[i].y; // works
    REP(i,sz)  a += cross( P[i] , P[ (i+1)%sz ] ,P[0]); // works 
    a = fabs(a) * 0.5;
...
int main()
{
    ...
    while(scanf("%d",&n) && n )
    {
     ...
     /*   for( i = 0 ; i< hullPoints.size() ;i++ )
        ...
        DB convex_area = calc_areaofConvex(hullPoints) ;
        DB area   = calc_areaofConvex(xy);
      //  cout << convex_area << "   " << area << endl;
        printf("Tile #%d\n",cs++);
